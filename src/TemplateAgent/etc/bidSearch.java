...
{	
	...
	public bidSearch(AbstractUtilitySpace utilitySpace, negotiationInfo negotiationInfo, boolean isPrinting) throws Exception {
	...
	private Bid getBidbyAppropriateSearch(Bid baseBid, double threshold) {
		...
		try {
			...
			if(negotiationInfo.isLinerUtilitySpace()){
				...
				if(utilitySpace.getUtility(bid) < threshold){ negotiationInfo.utilitySpaceTypeisNonLiner(); } // 探索に失敗した場合，非線形効用空間用の探索に切り替える
			} 
